...
from flectra import _, api, fields, models, service
...
class WindcaveTransaction(models.Model):
    ...
    def do_query(self):
        flectra_transaction = self.env['payment.transaction'].search([('reference', '=', self.reference)])
        
        result = self.query_transaction(self.transaction_id, 0)
        if result == True:
            if self.check_transaction_set(self.order_id, self.transaction_type):
                ...
                if self.transaction_type == 'purchase' or self.transaction_type == 'complete':
                    flectra_transaction._set_transaction_done()
                elif self.transaction_type == 'auth':
                    flectra_transaction._set_transaction_authorized()
                    ...
                    flectra_transaction.write({'windcave_amount_refunded': flectra_transaction.amount - new_balance})
                    if new_balance == 0:
                        flectra_transaction.write({'windcave_fully_refunded': True})
                flectra_transaction.update({'state_message':'This transaction was approved.'})
            else:
                _logger.info('Transaction declined.')
                if self.transaction_type == 'purchase' or self.transaction_type == 'auth':
                    flectra_transaction._set_transaction_cancel()
                flectra_transaction.update({'state_message':'This transaction was declined.'})
        elif result == False:
            _logger.error('Transaction failed: Error occured during the transaction.')
            flectra_transaction._set_transaction_cancel()
            flectra_transaction.update({'state_message':'An error occurred during the transaction.'})
        elif result == 'pending':
            _logger.error('Transaction outcome unknown.')
            flectra_transaction.update({'state_message':'Error - transaction outcome unknown.'})

    def do_rebill(self, reference, transaction_type, amount, currency, card_id):
        flectra_transaction = self.env['payment.transaction'].search([('reference', '=', reference)])
        
        result = self.start_transaction(_helper.get_xid(self.env['ir.config_parameter'].sudo().get_param('web.base.url'), reference, transaction_type), transaction_type, reference, amount, currency, 0, card_id)
        if result == True:
    ...
    def do_refund(self, refund_amount_requested = None):
        self.ensure_one()
        ...
        result = self.start_transaction(_helper.get_xid(self.env['ir.config_parameter'].sudo().get_param('web.base.url'), self.reference, 'transaction'), 'refund', self.reference, refund_amount_requested, self.currency, 0)
        ...
        if result == True:
            ...
            _logger.error('Refund failed: Error occured during the transaction.')
            self.env['sale.order'].search([('name', '=', self.order_id)]).message_post(body="Error while refunding transaction. Please retry.")
        elif result == "pending":
    ...
    def do_complete(self):
        ...
        flectra_transaction = self.env['payment.transaction'].search([('reference', '=', self.reference)])
        result = self.start_transaction(_helper.get_xid(self.env['ir.config_parameter'].sudo().get_param('web.base.url'), self.reference, 'transaction'), 'complete', self.reference, self.amount, self.currency, 0)
        if result == True:
            ...
            _logger.error('Complete failed: Error occured during the transaction.')
            self.env['sale.order'].search([('name', '=', self.order_id)]).message_post(body="Error while capturing transaction. Please retry.")
        elif result == "pending":
    ...
    def do_void(self):
        ...
        result = self.start_transaction(_helper.get_xid(self.env['ir.config_parameter'].sudo().get_param('web.base.url'), self.reference, 'transaction'), 'void', self.reference, self.amount, self.currency, 0)
        if result == True:
            ...
            _logger.error('Void failed: Error occured during the transaction.')
            self.env['sale.order'].search([('name', '=', self.order_id)]).message_post(body="An error occurred while voiding. Void failed.")
        elif result == "pending":
    ...
    def start_transaction(self, x_id, transaction_type, reference, amount, currency, retries, card_id = None):
        ...
        acquirer = self.env['payment.acquirer'].search([('name', '=', 'Windcave')])
        ...
        response = requests.post(acquirer.windcave_api_url + '/v1/transactions', json=payload, headers=headers)
        ...
        if response.status_code == 200:
            ...
            if approved:
                ...
                self.add_transaction(response_data, 'approved', reference, self.transaction_id)
            elif response_data['type'] == 'void':
                _logger.info('Transaction declined.')
            else:
                _logger.info('Transaction declined. Adding record.')
                self.add_transaction(response_data, 'declined', reference, self.transaction_id)
            return True
            ...
            self.add_transaction(response_data, 'pending', reference, self.transaction_id)
            return self.query_transaction(response_data['id'], 0)
            ...
            if retries < self._retry_limit:
                ...
                return self.start_transaction(x_id, transaction_type, reference, amount, currency, retries + 1, card_id)
            else:
    ...
    def query_transaction(self, new_transaction_id, retries):
        ...
        response = requests.get(acquirer.windcave_api_url + '/v1/transactions/' + new_transaction_id, headers=headers)
        response_data = response.json()
        ...
        if response.status_code == 200:
            ...
            if approved:
                _logger.info('Transaction approved, adding record.')
                self.add_transaction(response_data, 'approved', response_data['merchantReference'], self.transaction_id)
            elif response_data['type'] == 'void':
                _logger.info('Transaction declined.')
            else:
                _logger.info('Transaction declined, adding record.')
                self.add_transaction(response_data, 'declined', response_data['merchantReference'], self.transaction_id)
            return True
        elif response.status_code == 202:
            if retries < self._retry_limit:
                time.sleep(5)
                _logger.info('Transaction outcome unknown. Querying transaction.')
                return self.query_transaction(new_transaction_id, retries + 1)
            else:
    ...
    def add_transaction(self, transaction_data, status, reference, parent_transaction_id=None):
        _logger.info('Setting transaction record of transaction ID %s - %s'%(transaction_data['id'], status))

        flectra_order = self.env['sale.order'].search([('name', '=', reference.split('-',1)[0])])
        acquirer = self.env['payment.acquirer'].search([('name', '=', 'Windcave')])
        ...
        if transaction.transaction_id == False:
            transaction = self.env['windcave.transaction'].create({
                ...
                'customer_id' : flectra_order.partner_id
                })
        ...
        if transaction_data.get('type') == 'purchase' or transaction_data.get('type') == 'auth':
            flectra_transaction.write({'windcave_result': status, 'acquirer_reference': transaction_data['id']})
            if status == 'approved':
                flectra_transaction.write({'windcave_payment_exists': True})

        flectra_transaction.write({'windcave_pending_transactions': self.get_pending_count(reference)})
